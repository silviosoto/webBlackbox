...
class bitacoracontroller extends Controller
{
    ...
    function ConsultarMeses(Request $request){
        ...
        $datos = DB::select(" SELECT distinct MONTH(`Fecha`) as ano FROM `bitacoras` WHERE `Id_dispositivo` = :idDisp and YEAR(`Fecha`)= :fecha",
    ...
    function ConsultarDias($idDisp,$fecha, $mes){
        $data_points = array();
        $datos = DB::select(" SELECT DAY(`Fecha`) as Dia, AVG(`Velocidad`) as velocidad FROM `bitacoras` WHERE YEAR(`Fecha`) = :fecha and `Id_dispositivo` = :idDisp and MONTH(`Fecha`) = :mes GROUP by DAY(Fecha) order by DAY(Fecha)",
    ...
    function ConsultarDia(Request $request){
        ...
        $datos = DB::select("SELECT DAY(`Fecha`) as Dia FROM `bitacoras` WHERE YEAR(`Fecha`) = :fecha and `Id_dispositivo` = :idDisp and MONTH(`Fecha`) = :mes GROUP by DAY(Fecha) order by DAY(Fecha)",
    ...
    function ConsultarHoras($idDisp,$fecha, $mes, $dia){
        $data_points = array();
        $datos = DB::select("SELECT `Velocidad`, `Hora`, `Latitud`,`Longitud` FROM `bitacoras` WHERE YEAR(`Fecha`) = :fecha and `Id_dispositivo` = :idDisp and MONTH(`Fecha`) = :mes and DAY(`Fecha`) = :dia",
    ...
    function ConsultarPeritaje($idDisp,$fecha, $mes, $dia){
        ...
        //SELECT * FROM `bitacoras` WHERE YEAR(`Fecha`) = 2018 and `	Id_dispositivo` = 13 and MONTH(`Fecha`) = 4 and DAY(`Fecha`) = 5 AND HOUR(`Hora`)= (SELECT MAX(HOUR(`Hora`)) FROM `bitacoras` WHERE HOUR(`Hora`) )
}
